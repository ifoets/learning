...
public class ClientSideLoadBalancingTest {
	...
	public void balanceTest() {
		for (int i = 0; i < 100; i++) {
			BalanceCheckRequest balanceCheckRequest = BalanceCheckRequest.newBuilder()
			        .setAccountNumber(ThreadLocalRandom.current()
			                .nextInt(1, 11))
			        .build();
	...
	public void cashStreamingRequest() throws InterruptedException {
		...
		StreamObserver<DepositeRequest> streamObserver = this.bankServiceStub
		        .cashDeposite(new BalanceStreamObserver(latch));
		for (int i = 0; i < 10; i++) {
			DepositeRequest depositRequest = DepositeRequest.newBuilder()
			        .setAccountNumber(8)
			        .setAmount(10)
			        .build();
